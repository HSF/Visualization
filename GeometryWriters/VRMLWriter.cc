...
bool VRMLWriter::doit(std::string outputFilename)
{
  ...
  G4VPhysicalVolume* topVol = null;
  for (G4PhysicalVolume* physVol : *pvStore) {
    if (physVol->GetMotherLogical()) { continue; } // physVol is not the world volume
    topVol = physVol;
    break;
  }
  if (topVol == null) { return false; }
  ...
  if (outputFilename.length() > 0) {
    ...
    m_File.open("geometry.wrl", std::ios_base::trunc);
  }
  if (m_File.fail()) { return false; }

  // Write the VRML preamble and headers
  writePreamble();
...
void VRMLWriter::assignName(std::vector<std::string>* names, unsigned int index, const G4String& originalName, int select)
{
  G4SolidStore* solidStore = G4SolidStore::GetInstance();
  G4LogicalVolumeStore* lvStore = G4LogicalVolumeStore::GetInstance();
  G4PhysicalVolumeStore* pvStore = G4PhysicalVolumeStore::GetInstance();
...
void VRMLWriter::describeSolid(G4VSolid* solid, const std::string& solidName, bool isCylinder)
{
...
void VRMLWriter::describeLogicalVolume(G4LogicalVolume* logVol, const std::string& lvName, const std::string& solidName,
                                             bool isCylinder)
{
  G4LogicalVolumeStore* lvStore = G4LogicalVolumeStore::GetInstance();
  int lvIndex = std::find(lvStore->begin(), lvStore->end(), logVol) - lvStore->begin();
  ...
  if (materialName == "CDCGas") visible = "#"; // BelleII
  if (materialName == "ColdAir") visible = "#"; // BelleII
  if (materialName == "STR-DryAir") visible = "#"; // BelleII
  if (materialName == "TOPAir") visible = "#"; // BelleII
  if (materialName == "TOPVacuum") visible = "#"; // BelleII
...
void VRMLWriter::describePhysicalVolume(G4VPhysicalVolume* physVol)
{
  G4LogicalVolume* logVol = physVol->GetLogicalVolume();
  G4VSolid* solid = logVol->GetSolid();
  G4LogicalVolumeStore* lvStore = G4LogicalVolumeStore::GetInstance();
...
void VRMLWriter::descendAndDescribe(G4VPhysicalVolume* physVol, const std::string& lvName, int replica)
{
  G4PhysicalVolumeStore* pvStore = G4PhysicalVolumeStore::GetInstance();
  G4LogicalVolumeStore* lvStore = G4LogicalVolumeStore::GetInstance();
  G4SolidStore* solidStore = G4SolidStore::GetInstance();
...
void VRMLWriter::writePreamble(void)
{
...
void VRMLWriter::writePolyhedron(const G4Polyhedron* polyhedron, const std::string& name)
{
  if (polyhedron) {
...
void VRMLWriter::writePhysicalVolume(const G4VPhysicalVolume* physVol, const std::string& pvName, const std::string& lvName,
                                           bool written)
{
  if (written) {
...
HepPolyhedron* VRMLWriter::getBooleanSolidPolyhedron(G4VSolid* solid)
{
  ...
  if (solid->GetEntityType() == "G4UnionSolid") {
    ...
    *result = polyhedronA->intersect(*polyhedronB);
    ...
    std::err << "getBooleanSolidPolyhedron(): Unrecognized boolean solid " << solid->GetName() <<
